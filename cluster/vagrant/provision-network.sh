...
function provision-network {
  ...
  ifconfig | grep -q ${DOCKER_BRIDGE} || {
    ...
    #add ip route rules such that all traffic for VMNet1 is routed through the Linux host.
    echo "Add ip route rules such that all traffic for VMNet1 is routed through the Linux host"
